...
class RAGManager:
    ...
    def __init__(
        self,
        embed_api_url: str = "http://localhost:11434/embed",
    ...
    def init_default_kb(self):
        ...
        json_path = "../GJB/rules.json"

        existing_kbs = self.knowledge_bases
        if default_kb_name in existing_kbs:
            return
    ...
    def _load_excel_row_by_row(self, file_path):
        """
        处理 Excel：将每一行转换为一个 Document 对象
        格式化为：
        列名1: 值
        列名2: 值
        ...
        ...
        docs = []
        try:
            # 读取 Excel，确保空值被填充为空字符串，避免 NaN 报错
            ...
            for index, row in df.iterrows():
                # 拼接这一行的内容
                # 比如：信号名称: 主发电机1输出电压; 信号ID: ELEC_GEN1...
                content_parts = []
                for col in columns:
                    val = str(row[col]).strip()
                    # 只有当单元格有值时才拼接到文本中，减少无用噪音
                    if val:
